...
describe('Deployment: ', function () {
	it('Should deploy the contract and setup conditions', async function () {
		...
		// send the new NFTs to the contract
		await sendNFT(client, aliceId, AccountId.fromString(contractId.toString()), newNftTokenId, generateSerials(1, 125));
		// send 25 NFTs to Operator so we can test staking method
		await sendNFT(client, aliceId, operatorId, newNftTokenId, generateSerials(126, 150));
	...
	it('Should test the staking method', async function () {
		client.setOperator(operatorId, operatorKey);
		// set NFT allowance
		...
		operatorNftAllowances.push({ tokenId: newNftTokenId, owner: operatorId, spender: AccountId.fromString(contractId.toString()) });

		// stake serials 126-150
	...
	it('Prepare for Staked Swap - associate old tokens to contract', async function () {
		client.setOperator(operatorId, operatorKey);
		...
		const result = await contractExecuteFunction(
			...
			[
				[
					legacyCollectionNFT_1_TokenId.toSolidityAddress(),
					legacyCollectionNFT_2_TokenId.toSolidityAddress(),
					legacyCollectionNFT_3_TokenId.toSolidityAddress(),
				],
			],
		);

		if (result[0]?.status?.toString() !== 'SUCCESS') {
			console.log('Failed to prepare for staked swap:', result);
			fail();
		}
...
describe('Access Checks: ', function () {
	it('Alice cant call sensitive methods', async function () {
		...
		if (result[0]?.status?.name != 'OwnableUnauthorizedAccount') {
	...
	it('Alice can call regular methods', async function () {
		...
		expect(Number(result[0].length)).to.be.equal(2);
		expect(Number(result[0][0])).to.be.equal(1);
		expect(Number(result[0][1])).to.be.equal(51);
	});
});
...
describe('Interaction: ', function () {
	it('Bob can Swap', async function () {
		...
		// set an FT allowance
		const ftAllowanceSet = await setFTAllowance(
			client,
			lazyTokenId,
			bobId,
			AccountId.fromString(contractId.toString()),
		...
		expect(ftAllowanceSet).to.be.equal('SUCCESS');
		...
		// show tx Id
		console.log('Bob Swap (single) Tx ID:', result[2]?.transactionId?.toString());
		...
		expect(Number(bobLazyBal)).to.be.equal(100 + Number(origBobLazyBal));
		expect(Number(bobNFTBalance)).to.be.equal(1);
		expect(Number(bobLegacy1Bal)).to.be.equal(11);
		expect(Number(bobLegacy2Bal)).to.be.equal(12);
		expect(Number(bobLegacy3Bal)).to.be.equal(12);

		let aliceLegacy1Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_1_TokenId);
		expect(Number(aliceLegacy1Bal)).to.be.equal(14);


		// check contract balance
		let contractNFTBalance = await checkMirrorBalance(env, contractId, newNftTokenId);
		expect(Number(contractNFTBalance)).to.be.equal(149);
		...
		// show tx Id
		console.log('Bob Swap (multi) Tx ID:', result[2]?.transactionId?.toString());
		...
		console.log('Bob Balances:', newBobLazyBal, bobLazyBal);
		expect(Number(newBobLazyBal)).to.be.greaterThanOrEqual(3500 + Number(bobLazyBal));
		expect(Number(newBobNFTBalance)).to.be.equal(36);
		expect(Number(newBobLegacy1Bal)).to.be.equal(0);
		expect(Number(newBobLegacy2Bal)).to.be.equal(0);
		expect(Number(newBobLegacy3Bal)).to.be.equal(0);

		aliceLegacy1Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_1_TokenId);
		const aliceLegacy2Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_2_TokenId);
		const aliceLegacy3Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_3_TokenId);

		expect(Number(aliceLegacy1Bal)).to.be.equal(25);
		expect(Number(aliceLegacy2Bal)).to.be.equal(25);
		expect(Number(aliceLegacy3Bal)).to.be.equal(25);

		contractNFTBalance = await checkMirrorBalance(env, contractId, newNftTokenId);
		expect(Number(contractNFTBalance)).to.be.equal(114);
	...
	it('Operator can Swap', async function () {
		...
		expect(Number(operatorLazyBal)).to.be.equal(100 + Number(origOperatorLazyBal));
		expect(Number(operatorNFTBalance)).to.be.equal(1);
		expect(Number(operatorLegacy1Bal)).to.be.equal(24);
		expect(Number(operatorLegacy2Bal)).to.be.equal(25);
		expect(Number(operatorLegacy3Bal)).to.be.equal(25);

		let aliceLegacy1Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_1_TokenId);
		expect(Number(aliceLegacy1Bal)).to.be.equal(26);

		// check contract balance
		let contractNFTBalance = await checkMirrorBalance(env, contractId, newNftTokenId);
		expect(Number(contractNFTBalance)).to.be.equal(113);
		...
		console.log('Operator Balances:', newOperatorLazyBal, operatorLazyBal);
		expect(Number(newOperatorLazyBal)).to.be.greaterThanOrEqual(7400 + Number(operatorLazyBal));
		expect(Number(newOperatorNFTBalance)).to.be.equal(75);
		expect(Number(newOperatorLegacy1Bal)).to.be.equal(0);
		expect(Number(newOperatorLegacy2Bal)).to.be.equal(0);
		expect(Number(newOperatorLegacy3Bal)).to.be.equal(0);

		aliceLegacy1Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_1_TokenId);
		const aliceLegacy2Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_2_TokenId);
		const aliceLegacy3Bal = await checkMirrorBalance(env, aliceId, legacyCollectionNFT_3_TokenId);

		expect(Number(aliceLegacy1Bal)).to.be.equal(50);
		expect(Number(aliceLegacy2Bal)).to.be.equal(50);
		expect(Number(aliceLegacy3Bal)).to.be.equal(50);

		contractNFTBalance = await checkMirrorBalance(env, contractId, newNftTokenId);
		expect(Number(contractNFTBalance)).to.be.equal(39);
	});
...
async function sendLazy(receiverId, amt) {
	...
	if (result[0]?.status?.toString() !== 'SUCCESS') {
		...
		console.log('Receiver:', receiverId.toString(), 'Amt:', amt);
		fail();
	}
