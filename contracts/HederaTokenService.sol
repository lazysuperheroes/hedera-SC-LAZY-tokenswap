...
abstract contract HederaTokenService is HederaResponseCodes {
    ...
    function getApproved(
        address token,
        uint256 serialNumber
    ) internal returns (int256 responseCode, address approved) {
        (bool success, bytes memory result) = precompileAddress.call(
            abi.encodeWithSelector(
    ...
    function getTokenDefaultFreezeStatus(
        address token
    ) internal returns (int256 responseCode, bool defaultFreezeStatus) {
        (bool success, bytes memory result) = precompileAddress.call(
            abi.encodeWithSelector(
    ...
    function getTokenDefaultKycStatus(
        address token
    ) internal returns (int256 responseCode, bool defaultKycStatus) {
        (bool success, bytes memory result) = precompileAddress.call(
            abi.encodeWithSelector(
    ...
    /// @return responseCode The response code for the status of the request. SUCCESS is 22.
    function pauseToken(address token) external returns (int256 responseCode) {
        (bool success, bytes memory result) = precompileAddress.call(
            abi.encodeWithSelector(
